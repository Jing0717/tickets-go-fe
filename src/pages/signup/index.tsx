...
import { useRegisterUserMutation, useHandleGoogleMutation } from '@/store/authApi'
...
const Signup: React.FC = () => {
  ...
  const handleGoogleRegister = async (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault()
    try {
      await handleGoogle().unwrap()
      router.push('/google/callback')
    } catch (error) {
      toast.error('Google 註冊失敗，請重新操作一次')
    }
  }
      ...
      <div className='flex justify-center items-center min-h-screen bg-[#FFF5E1] w-full overflow-hidden'>
        <div className='relative flex justify-center items-center w-[800px] h-[800px]'>
          ...
          <div className='relative z-20 flex flex-col items-center justify-center gap-5 w-[300px] h-full'>
            ...
            <form className='flex flex-col items-center justify-center gap-5 w-[300px]' onSubmit={handleSignup}>
