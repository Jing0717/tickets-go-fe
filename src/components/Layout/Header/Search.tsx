...
const Search: React.FC<InputProps> = ({ type = 'text', placeholder = '', value, onChange }) => {
            ...
            <div className="max-w-[351px] mt-4 bg-[#4A4A4A] text-white absolute left-0 top-12 z-50 text-sm p-4">
              ...
              <div className="flex flex-wrap mt-2">
                {['大港開唱', '大港開唱', '滅火器', '滅火器', '吳卓元', '吳卓元'].map((tag, index) => (
                  <span key={index} className="flex items-center text-[#D9D9D9] pl-4 pr-3 py-2 border-[#D9D9D9] border-[1px] mr-2 mb-2">
                    {tag}
                    <FontAwesomeIcon
                      icon={faTimes}
                      className="w-4 h-4 text-white cursor-pointer ml-1"
                    />
                  </span>
                ))}
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex flex-col items-center p-4 bg-[#4A4A4A] text-white">
          ...
          <div className="w-full mt-4">
            ...
            <div className="flex flex-wrap mt-2">
              {['大港開唱', '大港開唱', '滅火器', '滅火器', '吳卓元', '吳卓元'].map((tag, index) => (
                <span key={index} className="flex items-center text-[#D9D9D9] px-2 py-1 border-[#D9D9D9] border-2 mr-2 mb-2">
                  {tag}
                  <FontAwesomeIcon
                    icon={faTimes}
